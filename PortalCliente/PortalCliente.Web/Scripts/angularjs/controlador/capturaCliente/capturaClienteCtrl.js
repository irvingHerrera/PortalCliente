...
(function () {
    ...
    capturaCliente.$inject = ['$scope', '$registroClienteService', '$capturaClienteService', '$catalogoService', '$q'];
     ...
     function capturaCliente($scope, $registroClienteService, $capturaClienteService, $catalogoService, $q) {
        ...
        var tempestado;
        var tempciudad;
        var estatus_auxiliar = 0;
        var vm = $scope;
        vm.inputDisable = true;
        vm.inputDisableDoc = true;
        $('#pasot2').prop('disabled', true); 
        $('#pasot4').prop('disabled', true); 
        ...
        vm.obtenerListaAprobador = function (precliente) {
            $capturaClienteService.obtenerListaAprobador({ idPrecliente: precliente.IdPrecliente })
                .then(function (data) {
        ...
        // function capturaCliente($scope, $registroClienteService, $capturaClienteService, $catalogoService, $q) {
        vm.ValidaNumeroDocumentosRevisados = function (id_precliente) {
            $capturaClienteService.ValidaNumeroDocumentosRevisados({ id_precliente: id_precliente }).then(function (data) {
                if (data.data.resultado == true) {
                    $('#pasot2').prop('disabled', false); // Se habilita la pestaña de "Tabuladores" en caso de existir datos en "Registro Cliente".
                    $('#pasot4').prop('disabled', false); // Se habilita la pestaña de "Tabuladores" en caso de existir datos en "Registro Cliente".
                }
                else {
                    //$('#pasot2').prop('disabled', true); // Se deshabilita la pestaña de "Tabuladores" en caso de no existir datos en "Registro Cliente".
                    //$('#pasot4').prop('disabled', true); // Se deshabilita la pestaña de "Tabuladores" en caso de no existir datos en "Registro Cliente".
                }
            }, function (error) {
                console.log("Error", error);
            })
        ...
        $scope.$on("seleccion_busqueda", function (evet, data) {
            $('.correcdoc').fadeOut('fast');
            vm.inputDisable = true; // Solo lectura
            vm.inputDisableDoc = true; // Solo lectura
            cargarRegistroCliente(data);
            $("#cont-paso4").css("display", "none");
            $('#pasot2').prop('disabled', true); // Se habilita la pestaña de "Tabuladores" en caso de existir datos en "Registro Cliente".
            $('#pasot4').prop('disabled', true); // Se habilita la pestaña de "Procedimientos de Operación" 
            $('#cont-pasot1').fadeIn('fast');
            $('#cont-pasot2').fadeOut('fast');
            $('#cont-pasot4').fadeOut('fast');
            $("#pasot1").addClass("btn-operaciones active");
            $("#pasot2").removeClass("btn-operaciones active").addClass("btn-operaciones");
            $("#pasot4").removeClass("btn-operaciones active").addClass("btn-operaciones");
            $("#terminar2-1").css("display", "");

            // Inhabilitamos los botones de guardar de "registro cliente" y "tabuladores"
            // en caso de ser "estatus 5" el precliente consultado.
            if (data.Estatus === 5) {
                $("#botonGuardarTabulador").prop("disabled", true);
                $("#terminar2-1").prop("disabled", true);
                $('#pasot2').prop('disabled', false); // Se habilita la pestaña de "Tabuladores" en caso de existir datos en "Registro Cliente".
                $('#pasot4').prop('disabled', false); // Se habilita la pestaña de "Procedimientos de Operación" 
                $('#cont-pasot1').fadeOut('fast');
                $('#cont-pasot2').fadeOut('fast');
                $('#cont-pasot4').fadeIn('fast');
                $("#cont-paso4").css("display", "");
                $("#pasot4").addClass("btn-operaciones active");
                $("#pasot1").removeClass("btn-operaciones active").addClass("btn-operaciones");
            }
            else if (data.Estatus === 12) {
                $("#botonGuardarTabulador").prop("disabled", true);
                $("#terminar2-1").prop("disabled", true);
                $('#pasot1').prop('disabled', false);
                $('#pasot2').prop('disabled', false);
                $('#pasot4').prop('disabled', false);
                $('#cont-pasot1').fadeOut('fast');
                $('#cont-pasot2').fadeOut('fast');
                $('#cont-pasot4').fadeIn('fast');
                $("#pasot4").addClass("btn-operaciones active");
                ...
                $("#botonGuardarTabulador").prop("disabled", false);
                $("#terminar2-1").prop("disabled", false);
            }
            ...
            vm.classTopAprobador = '';
            vm.listaAprobador = [];
            if (data.Estatus === 4 || data.Estatus === 12 || data.Estatus === 13)
                vm.obtenerListaAprobador(data);
            else
                vm.listaAprobador = [];

            // 12 y 13 para lider o gerente
            // 4 solo para aprobador y gerente
            ...
            $capturaClienteService.GuardarRevisionDocumentosCliente({ id_precliente: idPrecliente, id_usuario: usuario }).then(function (data) {
                ...
                if (data.data.resultado) {
                    ...
                    if (vm.Resultado.Resultado == 1) {
                        $("#pasot2").addClass("btn-operaciones active");
                        $("#pasot1").removeClass("btn-operaciones active").addClass("btn-operaciones");
                        $('#pasot2').prop('disabled', false); // Se habilita la pestaña de "Tabuladores" en caso de existir datos en "Registro Cliente".
                        //$('#cont-pasot1').fadeOut('fast');
                        //$('#cont-pasot2').fadeIn('fast');
                        //$('#cont-pasot4').fadeOut('fast');
                        var paso = '#cont-pasot' + 2;
                 ...
                 if (!validarTipoArchivo(vm.documentoSeleccionado.id_documento, extension)) {
                     ...
                     $capturaClienteService.guardarArchivo(formdata).then(function (resultado) {
                         if (resultado.data) {
                             ...
                             modalExitoConfirmacion2("El documento fue actualizado correctamente");
                         }
                         else {
        ...
        function modalPaso1(mensaje) {
            $('.popupoverlay').fadeIn('fast');
            $('#popupPaso1').fadeIn('fast');
            $('#popupPaso1 .icon i').addClass('far fa-check-circle');
            document.getElementById('mensajePaso1').innerHTML = mensaje;
        }
        ...
        function modalExitoConfirmacion2(mensaje) {
            $('.popupoverlay').fadeIn('fast');
            $('#popupFin').fadeIn('fast');
