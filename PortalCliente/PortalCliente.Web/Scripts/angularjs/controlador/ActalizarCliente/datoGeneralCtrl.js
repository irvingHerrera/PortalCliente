(function () {
    ...
    angular
        .module('app')
        .controller('datoGeneralCtrl', datoGeneral);

    datoGeneral.$inject = ['$scope', '$actualizaClienteService', '$capturaClienteService', '$catalogoService', '$q'];

    function datoGeneral($scope, $actualizaClienteService, $capturaClienteService, $catalogoService, $q) {
        var tempestado;
        var tempciudad;
        ...
        function obtenerIdPrecliente(idPrecliente) {
            var cero = '00000';
            cero = cero.substring(0, (cero.length - 1) - (idPrecliente.toString().length - 1));
            cero += idPrecliente.toString();
            return cero;
        }
        ...
        function resolverPromesa(promesaGeneral) {
            ...
            promesaGeneral.then(function (promesas) {
                ...
                if (promesas.promesaCapturaCliente.data.resultado) {
                    ...
                    vm.model.DatosCliente.IdPreclienteVisual = "PRE_" + obtenerIdPrecliente(vm.model.DatosCliente.IdPrecliente);
                ...
                if (promesas.promesaCuenta.data.resultado) {
                    ...
                    angular.forEach(cuentas, function (elemento, index) {
                        vm.model.DatosCliente.Cuenta.push({
                            ...
                            UsuarioCreacion : vm.model.DatosCliente.IdUsuario
                        });
                    })
                } else {
        ...
        vm.ValidaNumeroDocumentosRevisados = function (id_precliente) {
            $capturaClienteService.ValidaNumeroDocumentosRevisados({ id_precliente: id_precliente }).then(function (data) {
                if (data.data.resultado == true) {
                    ...
                    $('#paso2').prop('disabled', true); // Se deshabilita la pestaña de "Tabuladores" en caso de no existir datos en "Registro Cliente".
                }
            }, function (error) {
                console.log("Error", error);
            })
        ...
        $scope.$on("seleccion_busqueda", function (evet, data) {
            ...
            //console.log("seleccion_busqueda vvv");
        });
        ...
        vm.guardarEdicion = function () {
            ...
            $capturaClienteService.guardarEdicion(vm.model).then(function (data) {
                ...
                if (data.data.resultado) {
                    //actualizar estatus
                    modalExitoConfirmacion("Se guardo correctamente la información");
                    $('#paso2').prop('disabled', false); 
                }
                else {
                    modalError("Ocurrió un problema al actualizar la información");
                }
